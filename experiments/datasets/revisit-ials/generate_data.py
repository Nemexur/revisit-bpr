...
def generate_data(raw_data, output_dir, n_heldout_users, min_uc, min_sc):
    ...
    sparsity = 1.0 * raw_data.shape[0] / (user_activity.shape[0] * item_popularity.shape[0])
    print(
        "After filtering, there are %d watching events from %d users and %d "
        "movies (sparsity: %.3f%%)"
...
def main():
    ...
    parser.add_argument(
        "--output_dir", type=str, default="", help="Path where to save the datasets."
    )
    ...
    generate_data(raw_data, output_dir=ml20m_dir, n_heldout_users=10000, min_uc=5, min_sc=0)
    ...
    os.remove(msd_zip)
    raw_data = pd.read_csv(msd_file, sep="\t", header=None, names=["userId", "movieId", "count"])
    os.remove(msd_file)
    generate_data(raw_data, output_dir=msd_dir, n_heldout_users=50000, min_uc=20, min_sc=200)
